...
define('PASSWORD', 'adec');
define('PASSWORD_SALT', 'P5`6]NdaALYR}');
...
$footer = '<a href="http://github.com/mustafa0x/pafm">pafm v'.VERSION.'</a> '
	. 'by <a href="http://mus.tafa.us">mustafa</a><br>Maximum upload size: <strong>'.ini_get('upload_max_filesize').'</strong>';
...
function getFs($file){
	...
	elseif (filesize($file) <= 1024000)
		return round(filesize($file)/1024, 2).' <b title="KiloBytes" style="background-color: #FCFAC4;border-radius:4px;">KB</b>';
	else
		return round(filesize($file)/1024000, 2).' <b title="MegaBytes" style="background-color: #FDC5C3;border-radius:4px;">MB</b>';
}
function rrd($dir){
	$handle = opendir($dir);
...
function getFiles($path){
	...
	foreach ($dirContents['files'] as $dirItem){
		...
		// If the file is hidden, grey it out
		$extraclass=""; $extratext="";
		if(substr($dirItem, 0, 1)==".") {
			$extraclass=" hidden_file";
			$extratext='<span class="hidden_file">(hidden)</span>';
		}
		
		
		echo '  <li title="' . $dirItemHTML . '">' .
		"\n\t" . '<a href="' . escape(ROOT . $filePath . $dirItem) . '" title="' . $dirItemHTML . '" class="file'.$extraclass.'">'.$dirItemHTML.'</a> '.$extratext .
...
<head>
  ...
  .panel-heading span, .panel-heading span#file, .panel-heading span#fileop {
	font-size:1.5em;
	font-weight:800;
  }
  body{
		...
		<div class="col-lg-8">
			<div class="panel panel-default" style="margin-top:20px;">
					...
					<div id="dirList">
						<ul id="info">
						  <li>
							...
							<span class="filemtime">modified</span>
				...
				<div class="panel-body">
					...
					<div style="background-color:#FFFFD4;padding:10px;margin-top:50px;border-radius:20px;">
					<h2>Instructions</h2>
					<p style="font-size:19px;">
					You may create folders in the top directory only. A directory should refer to only one locaiton of data. Data files should be text or csv files located in the directories. Raw data from the recorders is fine.<br>
					</p>
					<hr style="width:80%">
					<p style="font-size:16px;">
					<strong>Example folder name:</strong> by_battalion <em>(which describes the location)</em> &mdash; Do not use spaces or special characters in the folder names<br>
					<strong>Example data file name:</strong> 0126-0205.txt <em>(which describes the time of the recording. Jan 26 - Feb 5)</em>&mdash; Files may be hidden by adding a "<strong>.</strong>" before the filename.<br>
					</p>
					</div>
				</div>
			</div>
